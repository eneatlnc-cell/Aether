...
> **版本**：v3.0（当前实现 v3.6：BSC 单链，Arbitrum 支持移除）
> **日期**：2026-07-24
> **链**：BNB Smart Chain（BSC，chainId 56）
> **状态**：Phase 1-6 开发完成；v3.1 修复 C2/H11/C1，v3.4 外部审计 Critical/High 全部修复，v3.6 完成 BSC 单链迁移；待主网部署
...
│       └──────────────┴──────┬───────┴──────────────┘          │
│                              ▼                                │
│         总支持率 = 三院支持率（合计 4998 BPS）+ 公民支持率（5000 BPS）│
│         通过门槛 > 50%（PASS_THRESHOLD_BPS = 5000）           │
│         制衡：三院全 FOR(4998)+公民 0%=4998 < 5000，三院无法独断  │
...
- **加权归一**：三院各 1666 BPS（合计 4998）+ 公民 5000 BPS，合计 9998 ≈ 10000（50/50 制衡）
...
- **通过条件**：公民参与率 ≥30% **且** 支持弹劾率（`citizenFor / citizenVotes`）≥70%
- **v3.1 修正**：原 `citizenAgainst`（语义反转，C2 bug）已改为 `citizenFor`
...
详见 [V3_AUDIT_REPORT.md](./V3_AUDIT_REPORT.md)。3 个 Critical（C2 已修复）+ 11 个 High（H11 已修复）。
...
| 参数 | 值 | 可调 |
|---|---|---|
| 三院权重 | 各 1666 BPS（合计 4998，≈50%）| 否（常量）|
| 公民权重 | 5000 BPS（50%）| 否（常量）|
| 通过门槛 | >50%（5000 BPS）| 否（常量）|
| 普通 quorum | 20% | 否（常量）|
| 章程 quorum | 50% | 否（常量）|
| 一审时长 | 5 天 | 是（PARAM）|
| 公投时长 | 7 天 | 是（PARAM）|
...
> 本白皮书基于 v3.1 代码库编写（含 v3.1 修复批次）。最新版本请参考 [GitHub 仓库](https://github.com/)。
